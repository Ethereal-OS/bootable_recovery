...
void ScreenRecoveryUI::draw_menu_and_text_buffer_locked(
  ...
  if (menu_) {
    auto& logo = fastbootd_logo_enabled_ ? fastbootd_logo_ : ethereal_logo_;
...
void ScreenRecoveryUI::draw_battery_capacity_locked() {
  int x;
  int y = margin_height_ + gr_get_height(ethereal_logo_.get());
...
bool ScreenRecoveryUI::Init(const std::string& locale) {
  ...
  if (android::base::GetBoolProperty("ro.boot.dynamic_partitions", false) ||
    ...
    ethereal_logo_ = LoadBitmap("logo_image_switch");
    fastbootd_logo_ = LoadBitmap("fastbootd");
  } else {
    ethereal_logo_ = LoadBitmap("logo_image");
  }
...
int ScreenRecoveryUI::SelectMenu(const Point& p) {
  ...
  if (menu_) {
    if (!menu_->IsMain()) {
      // Back arrow hitbox
      const static int logo_width = gr_get_width(ethereal_logo_.get());
      const static int logo_height = gr_get_height(ethereal_logo_.get());
